...
int main( /*int argc, char * argv[]*/ )
{
    ...
    *std::cout << ">>> ORIGINAL Vet = [ ";
    for( auto i(0) ; i < arrSz ; ++i )
        std::cout << V[i] << " ";
    std::cout << "], Size = " << arrSz << "\n\n";*/
    int V[] { -3,-44,6,9,-11,0,21,10,5,4 };
    bool (*fcnPtr)(int) { par_filter };
    size_t arrSz { sizeof(V)/sizeof(V[0]) };
    auto result = filter( V, V+arrSz, par_filter);
    std::cout << " V = [ " << *result;
    for (size_t i {1}; i < arrSz; i++)
    {
        if(!fcnPtr( *(result+i) )) break;
        std::cout << ", "<< *(result+i);
    }
    std::cout << " ]" << std::endl;
